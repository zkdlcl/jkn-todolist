...
### Phase 3.5: UI/UX 개선 (FE-12, FE-13) - 완료

1.  **상세 정보 툴팁 (FE-12)** ✅

    - 할일 이벤트 마우스 오버 시 Popover/Tooltip 표시
    - 표시 정보: 제목, 내용(일부), 마감일시, 우선순위
    - React Portal을 사용한 커스텀 Event 컴포넌트 구현
    - z-index 문제 해결 (Portal로 document.body에 렌더링)

2.  **주별/일별 보기 구현 (FE-13)** ✅
    - `react-big-calendar`의 Week, Day 뷰 활성화
    - 시간대별 일정 표시를 위한 allDay 플래그 자동 설정
    - 시작/마감 시간이 자정이 아닌 경우 시간대 블록으로 표시
    - 헤더 및 네비게이션 연동 확인
